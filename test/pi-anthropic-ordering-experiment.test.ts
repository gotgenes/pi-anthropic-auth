...
import { Type, getModel, streamSimple, type Context } from "@mariozechner/pi-ai"
...
test("experiment: current hook reshaping splits assistant tool_use blocks from trailing text", () => {
  const payload = {
    ...
    messages: [
      ...
      },
      {
        role: "assistant",
        content: [
          { type: "tool_use", id: "toolu_1", name: "Read", input: { filePath: "/root" } },
          { type: "tool_use", id: "toolu_2", name: "Glob", input: { pattern: "**/*.pdf" } },
      ...
      },
      {
        ...
        content: [
          ...
          { type: "tool_result", tool_use_id: "toolu_2", content: "No files found", is_error: false },
        ],
      },
    ],
    system: [
  ...
  const assistantMessages = shaped.messages.filter((message) => message.role === "assistant")

  assert.equal(assistantMessages.length, 2)
  assert.deepEqual(
    (assistantMessages[0]?.content as Array<{ type: string }>).map((block) => block.type),
    ["text"],
  )
  assert.deepEqual(
    (assistantMessages[1]?.content as Array<{ type: string }>).map((block) => block.type),
    ["tool_use", "tool_use"],
  )
})
